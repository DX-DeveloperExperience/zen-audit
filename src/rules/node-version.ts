...
export class NodeVersion {
	...
	constructor(rootPath: string = './') {
		...
		try {
			this.nodeVersionSchedule = JSON.parse(
				request('GET', 'https://raw.githubusercontent.com/nodejs/Release/master/schedule.json', {
					timeout: 3000
				})
					.getBody()
					.toString()
	...
	private isCritical() {
		...
		const now = Date.now();
		const maintenance = Date.parse(this.nodeVersionSchedule[this.shortNodeVersion].maintenance);
